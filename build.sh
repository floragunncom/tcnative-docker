...
do
   ...
   do
     ...
     docker run -e "NETTY_TCNATIVE_TAG=$nv" -e "LIBRESSL_VERSION=$LIBRESSL_VERSION" -e "LIBRESSL_SHA256=$LIBRESSL_SHA256" --rm -v "$DIR/$i/binaries:/output" "$i:latest" >> "$BUILDLOG" 2>&1
     VER=${nv##*-}
